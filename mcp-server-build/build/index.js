...
const SUPABASE_URL = 'https://bficcnxjytqijwyhkcfp.supabase.co'
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJmaWNjbnhqeXRxaWp3eWhrY2ZwIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc4MjEzNzQ2NywiZXhwIjoyMDk3NzEzNDY3fQ.k1ISZWAcxj5CpupnVg2eca0dNdR0tUMW6OeG7t10GX4'
...
function pkr(n) {
    if (n >= 1_000_000_000)
        return `Rs${(n / 1_000_000_000).toFixed(2)}B`;
    if (n >= 1_000_000)
        return `Rs${(n / 1_000_000).toFixed(2)}M`;
    if (n >= 1_000)
        return `Rs${(n / 1_000).toFixed(0)}K`;
    return `Rs${n}`;
}
...
async function getRows(uploadId) {
    const sb = getSupabase();
...
async function getTopSalesperson() {
    ...
    rows.forEach(r => {
        ...
        if (s)
            map[s] = (map[s] || 0) + num(r['Contract Value (PKR)']);
    });
    const sorted = Object.entries(map).sort((a, b) => b[1] - a[1]);
    if (!sorted.length)
        return 'No salesperson data found.';
...
// Server setup
const server = new Server({ name: 'fluxorx-mcp', version: '1.0.0' }, { capabilities: { tools: {} } });
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        if (name === 'get_overdue_invoices')
            result = await getOverdueInvoices();
        else if (name === 'get_top_salesperson')
            result = await getTopSalesperson();
        else if (name === 'get_city_breakdown')
            result = await getCityBreakdown();
        else if (name === 'get_delayed_projects')
            result = await getDelayedProjects();
        else if (name === 'get_collection_rate')
            result = await getCollectionRate();
        else if (name === 'get_completed_installations')
            result = await getCompletedInstallations();
        else if (name === 'get_system_capacity')
            result = await getSystemCapacity();
